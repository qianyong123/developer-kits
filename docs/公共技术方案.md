...
**当前状态：** 已接入 CloudBase 静态托管（`cloudbaserc.json` + GitHub Actions）：版本化部署（`tcb app deploy`）、仅手动触发（`workflow_dispatch`），部署需在 GitHub 仓库配置 `TENCENT_SECRET_ID` / `TENCENT_SECRET_KEY` secrets。v0.3 阶段以 CloudBase 为准，后续切换平台代码零改动。
...
| M7 部署接入 | CloudBase 静态托管（版本化部署、手动触发） | 可发布 |
